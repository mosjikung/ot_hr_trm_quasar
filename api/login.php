...
function ConnectDbNYGM($_sql){
 ...
 $app->post('/login_1', function (Request $request, Response $response) { // สร้าง route ขึ้นมารองรับการเข้าถึง url

  $dept = isset($_REQUEST['dept_code']) ? $_REQUEST['dept_code'] : '';
  $username = isset($_REQUEST['username']) ? $_REQUEST['username'] : '';
  
 
  
  
  $_sql = "SELECT * FROM DB_EMPLOYEE_V where EMP_CODE = '".$username."' and DEPT_CODE = '$dept'";
...
$app->post('/login_2', function (Request $request, Response $response) { // สร้าง route ขึ้นมารองรับการเข้าถึง url

  $dept_code = isset($_REQUEST['dept_code']) ? $_REQUEST['dept_code'] : '';

  
  $_sql = "select DEPT_EMP_APP , DIVISON_EMP_APP FROM TA_DEPARTMENT WHERE DEPT_CODE = '".$dept_code."'";
...
$app->post('/login_3', function (Request $request, Response $response) { // สร้าง route ขึ้นมารองรับการเข้าถึง url

  $username = isset($_REQUEST['username']) ? $_REQUEST['username'] : '';
  $dept = isset($_REQUEST['dept_code']) ? $_REQUEST['dept_code'] : '';

  
  $_sql = "select a.DEPT_CODE from  TA_DEPARTMENT a INNER JOIN  DB_EMPLOYEE_V b ON b.EMP_CODE = a.DEPT_EMP_APP or  b.EMP_CODE = a.DIVISON_EMP_APP 
  WHERE EMP_CODE ='".$username."' AND a.DEPT_CODE ='".$dept."'";
...
$app->post('/check_login_user_dept', function (Request $request, Response $response) { // สร้าง route ขึ้นมารองรับการเข้าถึง url

  $username = isset($_REQUEST['username']) ? $_REQUEST['username'] : '';
  

  
  $_sql = "SELECT * FROM TA_DEPARTMENT WHERE DEPT_EMP_APP = '".$username."' OR DIVISON_EMP_APP = '".$username."'";
